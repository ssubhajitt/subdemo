...
import pandas as pd#
import cloudant
import random# from cloudant
import Cloudant
from docx
import Document
from flask
import Flask, request, make_response, render_template, \
session, g
...
import PCA# from cloudant.error
import CloudantException# from cloudant.result
import Result, ResultByKey, QueryResult
...
url = 'https://' + host# client = Cloudant(user, password, url = url, connect = True)
app = Flask(__name__)
...
print hum# hum = par.get("humidity")
...
# print 'Username: {0}'.format(session['userCtx']['name'])# print 'Databases: {0}'.format(client.all_dbs())

# db = client['nwaveoutput']# doc = db.create_document(op)# doc.save()
...
print model_int# Data Processing
